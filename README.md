...
## Status
Built in disciplined **vertical slices** ([docs/BUILD-PLAN.md](docs/BUILD-PLAN.md), [CHANGELOG.md](CHANGELOG.md)). Done so far: secure shell + 10/Light·Medium·Dark themes · **Chat** (local Ollama + offline fallback, markdown) · **Office** — a staged home, Sheets/Docs/Slides editors, per-type **expert-agent AI build**, and real `.xlsx / .docx / .pptx / .pdf` export. Next: Knowledge/RAG.
